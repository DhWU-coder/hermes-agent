...
To attach a PDF or another file, paste it into Chat, drag it onto the terminal, or use **Add files** at the bottom of the pane. The browser uploads it to the selected profile and inserts an `@file:` reference into the TUI input; add your instructions and press Enter when ready. Images continue through the image attachment flow. If the browser exposes only a filename from the clipboard, use **Add files** to select the actual file bytes.
...
**Graphical chat (`/chat-ui`):** The dashboard also offers a browser-native chat page with a conversation list, streaming messages, a file composer, model selection, and a stop control. It connects to the existing Hermes session gateway. Starting another conversation or selecting a different one leaves running turns in the background; the list shows which sessions are working. The header shows the current task's elapsed time, and completed replies retain their final duration. Older replies without saved timing show an approximate duration when both message timestamps are available. The original `/chat` terminal remains available separately.
